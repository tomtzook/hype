...
extern void gdbTrace(const char* fmt, ...);
...
#define trace(fmt, ...) gdbTrace("[GDB] " fmt "\n", ##__VA_ARGS__)


inline unsigned char* strcpy(unsigned char* dst, const char* src) {
	  ...
	  if (checksum != xmitcsum)
	    {
	      if (remote_debug)
		{
		  trace("bad checksum.  My count = 0x%x, sent=0x%x. buf=%s\n", checksum, xmitcsum, buffer);
		}
...
/* These are separate functions so that they are so short and sweet
   ...
   saved).  */
int
get_char (const unsigned char *addr)
{
  return *addr;
}
...
unsigned char *
mem2hex (unsigned char* mem, unsigned char* buf, const int count, const int may_fault)
{
  int i;
...
handle_exception (int exceptionVector)
{
  ...
  if (remote_debug)
    {
      trace("vector=%d, sr=0x%x, pc=0x%x\n",
